'''
DBSCAN聚类可解决Kmeans聚类的两个缺点：一是聚类效果容易受到异常样本点的影响；二是无法准确的地将非球形样本进行合理的聚类。
层次聚类算法比较适合小样本的聚类，通过计算各个簇内样本点之间的相似度，构建一棵有层次的嵌套聚类树。
该算法不适合非球形样本的聚类，与Kmeans算法相似，人为设定聚类个数实现样本点的聚合。
'''
import pandas as pd
import numpy as np
...
# 读取外部数据
Province = pd.read_excel(r'C:\Users\Lenovo\Desktop\Province.xlsx')
Province.head()
# 绘制出生率与死亡率散点图
