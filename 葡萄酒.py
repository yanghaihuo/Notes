...
os.chdir(r'C:\Users\Lenovo\Desktop\Wine')
...
df = pd.read_csv(r'C:\Users\Lenovo\Desktop\Wine\winequality-white.csv',sep=';')
df.rename(columns={'fixed acidity':'非挥发性酸度','volatile acidity':'挥发性酸度','citric acid':'柠檬酸','residual sugar':'残留糖','chlorides':'氯化物','free sulfur dioxide':'游离二氧化硫','total sulfur dioxide':'总二氧化硫','density':'浓度','pH':'pH值','sulphates':'硫酸盐','alcohol':'酒精度','quality':'质量'},inplace=True)
...
df=df[df['质量'] > 6]
for i in df.columns:
    sns.countplot(x=i,hue='质量',data=df)
    plt.legend(loc='upper right')
    plt.show()


a=df[df.columns[:-1]].corrwith(df['质量'])
print(np.abs(a).sort_values(ascending=False))
sns.heatmap(df.corr(), annot = True,cmap = 'GnBu')
plt.show()

for i in df.columns[:-1]:
    sns.boxplot(x='质量',y=i,data=df)
    sns.stripplot(x=df['质量'],y=i,data=df)
    plt.show()

print(df['质量'].value_counts())
df['质量']=df['质量'].map({5:0,6:1})
...
# grid = sns.FacetGrid(df, row='浓度', col='氯化物',
#                      hue='质量', palette='seismic', size=4)
# grid.map(sns.countplot, '酒精度', alpha=0.8)
# grid.add_legend()
